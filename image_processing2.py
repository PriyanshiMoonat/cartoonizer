...
def cartoonify(ImagePath):
    ...
    getEdge = cv2.adaptiveThreshold(smoothGrayScale, 250, cv2.ADAPTIVE_THRESH_MEAN_C,cv2.THRESH_BINARY, 5, 5)

    ReSized4 = cv2.resize(getEdge, (940,610))
   
    #applying bilateral filter to remove noise and keep edge sharp as required
    colorImage = cv2.bilateralFilter(originalimage, 5, 250,250)
    ...
    fig, axes = plt.subplots(3,2, figsize=(5,5), subplot_kw={'xticks':[], 'yticks':[]}, gridspec_kw=dict(hspace=0.1, wspace=0.1))
    for i, ax in enumerate(axes.flat):
        ax.imshow(images[i], cmap='gray')
        
    # Make save button in main window
    save1=Button(top,text="Save cartoon image",command=lambda: save(ReSized6, ImagePath),padx=30,pady=5)
    save1.configure(background='#364156', foreground='white',font=('Times new roman',15,'bold'))
...
upload.configure(background='#364156', foreground='white',font=('Times new roman',15,'bold'))
